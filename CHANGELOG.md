...
- **Multiple DNS servers at build time** — interactive interview asks for DNS
  servers one at a time; an empty response finishes the list. CLI accepts
  repeated `--dns` or comma-separated values. All servers are applied via
  cloud-init (golden) and SSH post-install (ISO).
